...
ap.add_argument('--feats_drop_rate', type=float, default=0.4, help='The dropout of attribute completion.')
ap.add_argument('--ac_layers', type=int, default=2, help='layers of attribute completion. Default is 1.指定：HGAT_AC_R的注意力网络层的层数=ac_layers+1')
ap.add_argument('--lr', type=float, default=0.0005, help='学习率 lr 决定了每次迭代时参数更新的幅度。')
...
ap.add_argument('--ratio', type=int, default=0.11, help='选取出的同质节点的比率')
args = ap.parse_args()
print(args)
...
for cur_repeat in range(repeat):
    ...
    print(f"Model has {get_n_params(net)} trainable parameters.")
    ...
    with torch.no_grad():
        ...
        # color_idx = {}
        # for i in range(len(embeddings[test_idx])):
        ...
        # # plt.show()
        #
        # # 优化后代码 Database, Wireless Communication, Data Mining
        # color_map = {'1': '#DAA520', '2': '#8B0000', '0': '#6A5ACD', '3': '#006400'}
        # label_map = {'1': 'Wireless Communication', '2': 'Data Mining', '0': 'Database', '3': 'Class 3'}
        #
        # for c, idx in color_idx.items():
        #     plt.scatter(node_pos[idx, 0], node_pos[idx, 1], c=color_map[str(c)], s=15, alpha=1, label=label_map[str(c)])
        #
        # plt.legend(loc='upper right')  # 添加图例
        # plt.savefig("ACM_分类图.png", dpi=5000, bbox_inches='tight')  # 保存图片
        # plt.show()
