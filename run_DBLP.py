...
ap = argparse.ArgumentParser(description='NRSP testing for the DBLP dataset')
...
ap.add_argument('--feats_drop_rate', type=float, default=0.9, help='The dropout of attribute completion.')
ap.add_argument('--ac_layers', type=int, default=1, help='layers of attribute completion. Default is 1.指定：HGAT_AC_R的注意力网络层的层数=ac_layers+1')
ap.add_argument('--lr', type=float, default=0.005, help='学习率 lr 决定了每次迭代时参数更新的幅度。')
ap.add_argument('--weight_decay', type=float, default=0.001, help='权重衰减 weight_decay 是另一种正则化技术，它通过对权重施加 L2 正则化来防止模型过拟合。')
...
# # 数据加载
features_list, Relational_features_list, adjM, type_mask, labels, train_val_test_idx = load_DBLP_data()
